...
블로그 포스팅과 코드 업로도는 기본적으로 로컬 디렉토리에 brAIns 데이터베이스의 원격 저장소를 생성하여 연동하고, 연동된 로컬 디렉터리에서 brAIns 데이터베이스에 commit,push를 통해 업로드하는 방식으로 이루어집니다. (보다 자세한 원리가 궁금하신 분들은 [git의 원리](https://git-scm.com/book/ko/v2/%EC%8B%9C%EC%9E%91%ED%95%98%EA%B8%B0-%EB%B2%84%EC%A0%84-%EA%B4%80%EB%A6%AC%EB%9E%80%3F)를 더 읽어 보시면 좋을 것 같습니다.)   

초기 설정은 바로 이 원격 저장소를 로컬 디렉토리에 연동하는 과정을 진행하는 것이라고 보시면 될 것 같습니다.   
...
또는 아래 주소를 복사하셔도 됩니다.
    ...
    https://github.com/UNIST-brAIns/unist-brains.github.io.git
...
열린 command 창에 다음 코드를 입력하여 폴더를 git 저장소로 설정해줍니다.
    ...
    git init
다음 코드를 입력하여 로컬 디렉토리를 unist brAIns 저장소와 연동해줍니다.(복사한 주소) 위치에 ctrl-V를 통해 복사한 주소를 입력해줍니다. (괄호는 없어야 합니다!)
...
푸시 과정에서 github와 git bash 간의 연동 허용 창이 뜨기도 하는데, 허용해줍니다.
